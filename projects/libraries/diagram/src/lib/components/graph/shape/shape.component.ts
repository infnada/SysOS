...
export class ShapeComponent {
  ...
  UNIT_NETWORK_LEFT =
    'M 0.674 0 c 0.022 -0.022 0.017 -0.056 0 -0.078 l -0.104 -0.103 l -0.466' +
    ' -0.454 c -0.022 -0.022 -0.052 -0.022 -0.074 0 c -0.021 0.021 -0.026 0.056' +
    ' 0 0.077 l 0.489 0.479 c 0.021 0.022 0.021 0.056 0 0.078 l -0.497 0.497 c' +
    ' -0.022 0.021 -0.022 0.056 0 0.078 c 0.021 0.022 0.056 0.018 0.073 0 l 0.462' +
    ' -0.457 c 0 0 0 0 0.004 -0.004 L 0.674 0 z';

  UNIT_NETWORL_RIGHT =
    'M 0.1 0 c -0.022 -0.022 -0.017 -0.056 0 -0.078 l 0.104 -0.103 l 0.466 -0.454' +
    ' c 0.022 -0.022 0.052 -0.022 0.074 0 c 0.021 0.021 0.026 0.056 0 0.077 l -0.489' +
    ' 0.479 c -0.021 0.022 -0.021 0.056 0 0.078 l 0.497 0.497 c 0.022 0.021 0.022' +
    ' 0.056 0 0.078 c -0.021 0.022 -0.056 0.018 -0.073 0 l -0.462 -0.457 c 0 0 0 0' +
    ' -0.004 -0.004 L 0.1 0 z';
  ...
  <g transform="translate(-1.5)">
    <g transform="translate(2.2)">
        <path style="fill:#3999C6;" d="${this.UNIT_NETWORK_LEFT}"/>
    </g>
    <g>
        <path style="fill:#3999C6;" d="${this.UNIT_NETWORL_RIGHT}"/>
    </g>
  ...
  constructor(private readonly LibDiagramDomUtils: AnyOpsOSLibDiagramDomUtilsService) {
  }

  getClipId() {
    return this.LibDiagramDomUtils.encodeIdAttribute(`metric-clip-${this.nodeElement.id}`);
  }
  ...
  renderTemplate(type: string, allowStroke: boolean = true): string {
    const toReturn = this.shapes[this.nodeElement.shape].replace('#ATTRS', this.getAttrsByType(type).map(a => `${a.name}="${a.value}"`).join(''));
  ...
  renderHighlightSpape() {
    return `${this.nodeElement.highlighted ? `${this.renderTemplate('borderAttrs', false)}
    ${this.renderTemplate('shadowAttrs', false)}` : ''}`;
  }

  renderBaseShape(highlighted: boolean = this.nodeElement.highlighted) {
