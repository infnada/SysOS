...
import {AnyOpsOSVmwareModule} from './anyopsos-module-vmware.module';
...
export class AnyOpsOSVmwareFileSystemModule {

  private readonly GetPathModule: AnyOpsOSGetPathModule;
  private readonly VmwareModule: AnyOpsOSVmwareModule;
  ...
  constructor(private readonly userUuid: string,
    ...
    this.VmwareModule = new AnyOpsOSVmwareModule(this.userUuid, this.sessionUuid, this.workspaceUuid, this.connectionUuid);
  ...
  async uploadToDatastore(srcPath: string, datastoreUrl: string): Promise<void> {
    ...
    await spawn('curl', ['-k', '--user', `${mainServer.credential.fields.UserName}:${mainServer.credential.fields.Password.getText()}`, '-X', fileUrl, '-T', realSrcPath], { capture: [ 'stdout', 'stderr' ]});
  }
}
