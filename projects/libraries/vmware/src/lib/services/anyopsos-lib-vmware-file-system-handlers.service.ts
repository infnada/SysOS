...
export class AnyOpsOSLibVmwareFileSystemHandlersService {
  ...
  constructor(private readonly logger: AnyOpsOSLibLoggerService,
              ...
              private readonly VmwareFileSystem: AnyOpsOSLibVmwareFileSystemService) {
  }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.VmwareFileSystem.getFolder(data.srcPath, data.srcDatastoreName, data.datastoreBrowserName, data.connection.uuid).subscribe(
            (res: BackendResponse & { data: AnyOpsOSFile[]; }) => {
              if (res.status === 'error') {
                this.logger.error('LibVmware', 'UIgetFolder -> Error while getting folder', null, res.data);
            ...
            (error: any) => {
              this.logger.error('LibVmware', 'Error while getting fileSystemPath', null, error);
              return reject(error);
            });
        });
      });
        ...
        return new Promise(async (resolve, reject) => {
          ...
          modalInstance.afterClosed().subscribe((folderName: string) => {
            ...
            this.VmwareFileSystem.putFolder(data.srcPath, folderName, data.srcDatastoreName, data.srcDatacenterName, data.connection.uuid).subscribe(
              (res: BackendResponse) => {
                if (res.status === 'error') {
                  this.logger.error('LibVmware', 'UIputFolder -> Error while creating folder', null, res.data);
                  return reject(res.data);
                }
              ...
              (error: any) => {
                this.logger.error('LibVmware', 'UIputFolder -> Error while creating folder', null, error);
                return reject(error);
              });
          });
        });
        ...
        return new Promise(async (resolve, reject) => {
          ...
          modalInstance.afterClosed().subscribe((fileName: string) => {
            ...
            this.VmwareFileSystem.renameFile(data.srcPath, data.file.fileName, fileName, data.srcDatastoreName, data.srcDatacenterName, data.connection.uuid).subscribe(
              (res: BackendResponse) => {
                if (res.status === 'error') {
                  this.logger.error('LibVmware', 'UIrenameFile -> Error while renaming file', null, res.data);
                  return reject(res.data);
                }
              ...
              (error: any) => {
                this.logger.error('LibVmware', 'UIrenameFile -> Error while renaming file', null, error);
                return reject(error);
              });
          });
        });
        ...
        return new Promise(async (resolve, reject) => {
          ...
          modalInstance.afterClosed().subscribe((result: string) => {
            ...
            this.VmwareFileSystem.deleteFile(data.srcPath, data.file.fileName, data.srcDatastoreName, data.srcDatacenterName, data.connection.uuid).subscribe(
              (res: BackendResponse) => {
                if (res.status === 'error') {
                  this.logger.error('LibVmware', 'UIdeleteFile -> Error while deleting file', null, res.data);
                  return reject(res.data);
                }
              ...
              (error: any) => {
                this.logger.error('LibVmware', 'UIdeleteFile -> Error while deleting folder', null, error);
                return reject(error);
              });
          });
        });
      ...
      (data: { cutFile: CutCopyFile; srcDatastoreName: string; srcDatacenterName: string; dstPath: string; dstDatastoreName: string; dstDatacenterName: string; connection: ConnectionVmware; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.VmwareFileSystem.moveFile(
            ...
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibVmware', 'UIpasteFile -> Error while moving file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibVmware', 'UIpasteFile -> Error while moving file', null, error);
              return reject(error);
            });
        });
      });
      ...
      (data: { copyFile: CutCopyFile; srcDatastoreName: string; srcDatacenterName: string; dstPath: string; dstDatastoreName: string; dstDatacenterName: string; connection: ConnectionVmware; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.VmwareFileSystem.copyFile(
            ...
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibVmware', 'UIpasteFile -> Error while copying file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibVmware', 'UIpasteFile -> Error while copying file', null, error);
              return reject(error);
            });
        });
      });
      ...
      (data: { srcPath: string; dstPath: string; connection: ConnectionVmware; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.VmwareFileSystem.getFile(data.srcPath, data.dstPath, data.connection.uuid).subscribe(
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibVmware', 'UIpasteFile -> Error while downloading file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibVmware', 'UIpasteFile -> Error while downloading file', null, error);
              return reject(error);
            });
        });
      });
      ...
      (data: { srcPath: string; dstPath: string; connection: ConnectionVmware; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.VmwareFileSystem.putFile(data.srcPath, data.dstPath, data.connection.uuid).subscribe(
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibVmware', 'UIpasteFile -> Error while uploading file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibVmware', 'UIpasteFile -> Error while uploading file', null, error);
              return reject(error);
            });
        });
      });
