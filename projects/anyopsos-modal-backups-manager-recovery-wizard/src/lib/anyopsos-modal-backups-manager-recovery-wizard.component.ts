...
@Component({
  selector: 'smbmrw-anyopsos-modal-backups-manager-recovery-wizard',
  templateUrl: './anyopsos-modal-backups-manager-recovery-wizard.component.html',
  styleUrls: ['./anyopsos-modal-backups-manager-recovery-wizard.component.scss']
})
export class AnyOpsOSModalBackupsManagerRecoveryWizardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      ...
      // A VM is provided
         ...
         */
        this.Modal.openLittleModal('PLEASE WAIT', 'Searching VM in SnapShots...', '.modal-backups-manager-recovery-wizard', 'plain').then(() => {
          ...
          this.Modal.closeModal('.modal-backups-manager-recovery-wizard');
        }).catch((e) => {
          console.log(e);
  ...
  checkESXidata(): void {
    ...
    this.Modal.openLittleModal('PLEASE WAIT', 'Connecting to vCenter...', '.modal-backups-manager-recovery-wizard', 'plain').then(() => {
      ...
      this.Modal.changeModalText('Getting data...', '.modal-backups-manager-recovery-wizard');
      ...
      if (this.Modal.isModalOpened('.modal-backups-manager-recovery-wizard')) {
  ...
  checkIface(): void {
    ...
    if (this.selectedIface.info.data['data-protocols']['data-protocol'] === 'nfs' && !this.ifaceServiceData) {
      this.Modal.openLittleModal('PLEASE WAIT', 'Checking storage service status...', '.modal-backups-manager-recovery-wizard', 'plain').then(() => {
