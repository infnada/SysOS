...
@Component({
  ...
  templateUrl: './anyopsos-modal-monitor-export.component.html',
...
export class AnyOpsOSModalMonitorExportComponent implements OnInit {
  ...
  constructor(public readonly dialogRef: MatDialogRef<AnyOpsOSModalMonitorExportComponent>,
              @Inject(MAT_DIALOG_DATA) public readonly data: ModalData,
              private LibFileSystem: AnyOpsOSLibFileSystemService) {
  ...
  ngOnInit(): void {
    ...
    this.modalBody.title = this.data.title ?? 'Export a snapshot';
  ...
  private saveObjectToClient(data, filename) {
    ...
    if (this.saveAt === 'anyopsos') {
      ...
      return this.LibFileSystem.putFile('/home/root/Downloads/', file);
    }
