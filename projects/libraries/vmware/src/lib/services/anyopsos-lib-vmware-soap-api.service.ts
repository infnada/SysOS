...
export class AnyOpsOSLibVmwareSoapApiService {
  ...
  callSoapApi<Action extends VmwareSdkFunctions>(connectionUuid: string, action: Action, data: VmwareSdkFunctionsInput<Action>): Promise<Object> {
    return this.http.post(`/api/vmware/soap/${this.LibWorkspace.getCurrentWorkspaceUuid()}/${connectionUuid}`, {action, data}).toPromise();
  }
