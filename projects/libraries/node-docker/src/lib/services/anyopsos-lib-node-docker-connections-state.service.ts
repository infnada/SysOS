...
export class AnyOpsOSLibNodeDockerConnectionsStateService {
  private connectionsInitialized: boolean = false;

  readonly $connections: BehaviorSubject<ConnectionDocker[]>;
  private dataStore: {
  ...
  setConnectionsInitialized(): void {
    if (this.connectionsInitialized === true) {
      ...
      this.logger.error('LibNodeDocker', 'setConnectionsInitialized -> Connections already initialized');
  ...
  putConnection(connection: ConnectionDocker, saveConnection: boolean = true): Promise<void> {
    this.logger.debug('LibNodeDocker', 'New connection received', arguments);

    const connectionExists: ConnectionDocker = this.dataStore.connections.find((conn: ConnectionDocker) => conn.uuid === connection.uuid);
    if (connectionExists) {
      this.logger.error('LibNodeDocker', 'putConnection -> Resource already exists', arguments);
  ...
  patchConnection(connectionUuid: string, param: string, data: any): Promise<void> {
    ...
    if (connectionIndex === -1) {
      this.logger.error('LibNodeDocker', 'patchConnection -> Resource invalid', arguments);
  ...
  patchFullConnection(connection: ConnectionDocker): Promise<void> {
    ...
    if (connectionIndex === -1) {
      this.logger.error('LibNodeDocker', 'patchFullConnection -> Resource invalid', arguments);
      throw new Error('resource_invalid');
    }
  ...
  deleteConnection(connectionUuid: string): Promise<void> {
    ...
    if (!currentConnection) {
      this.logger.error('LibNodeDocker', 'deleteConnection -> Resource invalid', arguments);
      throw new Error('resource_invalid');
    }
  ...
  private saveBackend(currentConnection: ConnectionDocker, type: 'put' | 'patch' | 'delete'): Promise<void> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      fileSystemObservable.subscribe((res: BackendResponse) => {
          ...
          this.logger.debug('LibNodeDocker', 'Saved connection successfully', loggerArgs);
          return resolve(res.data);
        },
        error => {
          this.logger.error('LibNodeDocker', 'Error while saving connection', loggerArgs, error);
          return reject(error);
        });
    });
  }
