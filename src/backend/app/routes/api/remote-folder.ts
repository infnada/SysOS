 ...
 * Get folder contents
 */
router.get('/:uuid/:folderName(*)', async (req: express.Request, res: express.Response) => {
  ...
  SFTPWrapper.readdir(req.params.folderName, (e, data) => {
 ...
 * Create folder
 */
router.post('/:uuid/:folderName(*)', async (req: express.Request, res: express.Response) => {
  logger.info(`[API RemoteFolder] -> Creating folder ->  uuid [${req.params.uuid}], folder [${req.params.folderName}]`);

  const apiGlobals = new ApiGlobalsModule(req, res);
  const SFTPWrapper = await sshSessions.getSFTPWrapper(req.params.uuid);

  SFTPWrapper.mkdir(req.params.folderName, (e) => {
    if (e && e.code) return apiGlobals.serverError(e.code);
    if (e) return apiGlobals.serverError(e);
