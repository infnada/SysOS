...
export class AnyOpsOSLibNodeNetappConnectionsStateService {
  private connectionsInitialized: boolean = false;

  readonly $connections: BehaviorSubject<ConnectionNetapp[]>;
  private dataStore: {
  ...
  setConnectionsInitialized(): void {
    if (this.connectionsInitialized === true) {
      ...
      this.logger.error('LibNodeNetapp', 'setConnectionsInitialized -> Connections already initialized');
    ...
    this.LibFileSystem.getConfigFile(NETAPP_CONFIG_FILE)
      .subscribe((connectionsData: BackendResponse & { data: ConnectionNetapp[]; }) => {
        if (connectionsData.status === 'error') {
          this.logger.error('LibNodeNetapp', 'Error while initializing connections', null, connectionsData.data);
  ...
  putConnection(connection: ConnectionNetapp, saveConnection: boolean = true): Promise<void> {
    this.logger.debug('LibNodeNetapp', 'New connection received', arguments);

    const connectionExists: ConnectionNetapp = this.dataStore.connections.find((conn: ConnectionNetapp) => conn.uuid === connection.uuid);
    if (connectionExists) {
      this.logger.error('LibNodeNetapp', 'putConnection -> Resource already exists', arguments);
  ...
  patchConnection(connectionUuid: string, param: string, data: any): Promise<void> {
    ...
    if (connectionIndex === -1) {
      this.logger.error('LibNodeNetapp', 'patchConnection -> Resource invalid', arguments);
  ...
  patchFullConnection(connection: ConnectionNetapp): Promise<void> {
    ...
    if (connectionIndex === -1) {
      this.logger.error('LibNodeNetapp', 'patchFullConnection -> Resource invalid', arguments);
      throw new Error('resource_invalid');
    }
  ...
  deleteConnection(connectionUuid: string): Promise<void> {
    ...
    if (!currentConnection) {
      this.logger.error('LibNodeNetapp', 'deleteConnection -> Resource invalid', arguments);
      throw new Error('resource_invalid');
    }
