...
export class AnyOpsOSLibNetappFileSystemHandlersService {
  ...
  constructor(private readonly logger: AnyOpsOSLibLoggerService,
              ...
              private readonly NetappFileSystem: AnyOpsOSLibNetappFileSystemService) {
  }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.NetappFileSystem.getFolder(data.srcPath, data.connection.uuid, data.vfiler).subscribe(
            (res: BackendResponse & { data: AnyOpsOSFile[]; }) => {
              if (res.status === 'error') {
                this.logger.error('LibNetapp', 'UIgetFolder -> Error while getting folder', null, res.data);
            ...
            (error: any) => {
              this.logger.error('LibNetapp', 'Error while getting fileSystemPath', null, error);
              return reject(error);
            });
        });
      });
        ...
        return new Promise(async (resolve, reject) => {
          ...
          modalInstance.afterClosed().subscribe((folderName: string) => {
            ...
            this.NetappFileSystem.putFolder(data.srcPath, folderName, data.connection.uuid, data.vfiler).subscribe(
              (res: BackendResponse) => {
                if (res.status === 'error') {
                  this.logger.error('LibNetapp', 'UIputFolder -> Error while creating folder', null, res.data);
                  return reject(res.data);
                }
              ...
              (error: any) => {
                this.logger.error('LibNetapp', 'UIputFolder -> Error while creating folder', null, error);
                return reject(error);
              });
          });
        });
        ...
        return new Promise(async (resolve, reject) => {
          ...
          modalInstance.afterClosed().subscribe((fileName: string) => {
            ...
            this.NetappFileSystem.renameFile(data.srcPath, data.file.fileName, fileName, data.connection.uuid, data.vfiler).subscribe(
              (res: BackendResponse) => {
                if (res.status === 'error') {
                  this.logger.error('LibNetapp', 'UIrenameFile -> Error while renaming file', null, res.data);
                  return reject(res.data);
                }
              ...
              (error: any) => {
                this.logger.error('LibNetapp', 'UIrenameFile -> Error while renaming file', null, error);
                return reject(error);
              });
          });
        });
        ...
        return new Promise(async (resolve, reject) => {
          ...
          modalInstance.afterClosed().subscribe((result: string) => {
            ...
            this.NetappFileSystem.deleteFile(data.srcPath, data.file.fileName, data.connection.uuid, data.vfiler).subscribe(
              (res: BackendResponse) => {
                if (res.status === 'error') {
                  this.logger.error('LibNetapp', 'UIdeleteFile -> Error while deleting file', null, res.data);
                  return reject(res.data);
                }
              ...
              (error: any) => {
                this.logger.error('LibNetapp', 'UIdeleteFile -> Error while deleting folder', null, error);
                return reject(error);
              });
          });
        });
      ...
      (data: { cutFile: CutCopyFile; dstPath: string; connection: ConnectionNetapp; vfiler: string; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.NetappFileSystem.moveFile(
            ...
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibNetapp', 'UIpasteFile -> Error while moving file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibNetapp', 'UIpasteFile -> Error while moving file', null, error);
              return reject(error);
            });
        });
      });
      ...
      (data: { dstPath: string; copyFile: CutCopyFile; connection: ConnectionNetapp; vfiler: string; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.NetappFileSystem.copyFile(
            ...
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibNetapp', 'UIpasteFile -> Error while copying file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibNetapp', 'UIpasteFile -> Error while copying file', null, error);
              return reject(error);
            });
        });
      });
      ...
      (data: { srcPath: string; dstPath: string; connection: ConnectionNetapp; vfiler: string; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.NetappFileSystem.getFile(data.srcPath, data.dstPath, data.connection.uuid, data.vfiler).subscribe(
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibNetapp', 'UIpasteFile -> Error while downloading file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibNetapp', 'UIpasteFile -> Error while downloading file', null, error);
              return reject(error);
            });
        });
      });
      ...
      (data: { srcPath: string; dstPath: string; connection: ConnectionNetapp; vfiler: string; }
        ...
        return new Promise((resolve, reject) => {
          ...
          this.NetappFileSystem.putFile(data.srcPath, data.dstPath, data.connection.uuid, data.vfiler).subscribe(
            (res: BackendResponse) => {
              if (res.status === 'error') {
                this.logger.error('LibNetapp', 'UIpasteFile -> Error while uploading file', null, res.data);
                return reject(res.data);
              }
            ...
            (error: any) => {
              this.logger.error('LibNetapp', 'UIpasteFile -> Error while uploading file', null, error);
              return reject(error);
            });
        });
      });
