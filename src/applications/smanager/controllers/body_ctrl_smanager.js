            ...
            var initConnection = function (init) {
                ...
                if (_this.getActiveConnection().so === 'snmp') {
                    ...
                    var getInterfacesStatistics = function () {
                        ...
                        var diff;
                        var total_octets_in;
                        var total_octets_out;
                        var total_packets;
                        var total_packets_loss;

                        _this.getActiveConnection.data.totals.total_octets_in = 0;
                        _this.getActiveConnection.data.totals.total_octets_out = 0;
                        _this.getActiveConnection.data.totals.total_packets_sum = 0;
                        _this.getActiveConnection.data.totals.total_packets_loss_sum = 0;
                        _this.getActiveConnection.data.totals.total_speed = 0;

                        angular.forEach(_this.getActiveConnection.data.ifaces, function (iface) {
                            var ifaceOIDs = {
                                oid: [
                                    '1.3.6.1.2.1.2.2.1.5.' + iface.id, // ifSpeed
                                    '1.3.6.1.2.1.2.2.1.8.' + iface.id, // ifOperStatus
                                    '1.3.6.1.2.1.2.2.1.13.' + iface.id, // ifInDiscards
                                    '1.3.6.1.2.1.2.2.1.19.' + iface.id, // ifOutDiscards
                                    '1.3.6.1.2.1.2.2.1.14.' + iface.id, // ifInErrors
                                    '1.3.6.1.2.1.2.2.1.20.' + iface.id, // ifOutErrors
                                    '1.3.6.1.2.1.2.2.1.10.' + iface.id, // ifInOctets
                                    '1.3.6.1.2.1.2.2.1.16.' + iface.id, // ifOutOctets
                                    '1.3.6.1.2.1.2.2.1.11.' + iface.id, // ifInUcastPkts
                                    '1.3.6.1.2.1.2.2.1.17.' + iface.id, // ifOutUcastPkts
                                ],
                                type: 'get'
                            };
                            ...
                            oid_promises.push(ServerFactory.remoteDoSnmp(_this.activeConnection, ifaceOIDs.oid, ifaceOIDs.type).then(function (data) {
                                ...
                                // 1st fetch
                                if (angular.isUndefined(iface.init)) {

                                    iface.init = {};
                                    iface.statistics = {};
                                    iface.statistics.total_speed = 0;

                                    angular.forEach(data.data.data.data, function (stat) {
                                        iface.init[stat.oid] = parseInt(stat.value);
                                        iface.statistics[stat.oid] = [];
                                        iface.statistics.total_octets = [[{
                                            x: 0,
                                            y: 0
                                        }],[{
                                            x: 0,
                                    ...
                                    total_octets_out = 0;
                                    total_packets = 0;
                                    total_packets_loss = 0;

                                    angular.forEach(data.data.data.data, function (stat) {

                                        if (stat.oid.startsWith('1.3.6.1.2.1.2.2.1.5.')) return iface.info.speed = parseInt(stat.value);
                                        if (stat.oid.startsWith('1.3.6.1.2.1.2.2.1.8.')) return iface.init[stat.oid] = parseInt(stat.value);

                                        // Next fetchs
                                        // Unshift to array with calculated value
                                        diff = parseInt(stat.value) - iface.init[stat.oid];

                                        iface.statistics[stat.oid].unshift(diff);
                                        iface.init[stat.oid] = parseInt(stat.value);

                                        if (stat.oid.startsWith('1.3.6.1.2.1.2.2.1.10.')) {
                                            total_octets_in += diff;
                                            _this.getActiveConnection.data.totals.total_octets_in += diff;
                                        }
                                        if (stat.oid.startsWith('1.3.6.1.2.1.2.2.1.16.')) {
                                            total_octets_out += diff;
                                            _this.getActiveConnection.data.totals.total_octets_out += diff;
                                        }


                                        if (stat.oid.startsWith('1.3.6.1.2.1.2.2.1.11.') || stat.oid.startsWith('1.3.6.1.2.1.2.2.1.17.')) {
                                            total_packets += diff;
                                            _this.getActiveConnection.data.totals.total_packets_sum += diff;
                                        }

                                        if (stat.oid.startsWith('1.3.6.1.2.1.2.2.1.14.') || stat.oid.startsWith('1.3.6.1.2.1.2.2.1.20.')) {
                                            total_packets_loss += diff;
                                            _this.getActiveConnection.data.totals.total_packets_loss_sum += diff;
                                        }
                                    });

                                    // total iface octets
                                    iface.statistics.total_octets[0].unshift({
                                        x: iface.statistics.total_octets[0].length,
                                        y: total_octets_in
                                    });
                                    iface.statistics.total_octets[1].unshift({
                                        x: iface.statistics.total_octets[1].length,
                                        y: -total_octets_out
                                    });

                                    // Total speed
                                    // https://www.cisco.com/c/en/us/support/docs/ip/simple-network-management-protocol-snmp/8141-calculate-bandwidth-snmp.html
                                    iface.statistics.total_speed = parseFloat(((total_octets_in * 8 * 100) / (20 * iface.info.speed)) + ((total_octets_out * 8 * 100) / (20 * iface.info.speed)));

                                    _this.getActiveConnection.data.totals.total_speed += iface.statistics.total_speed;

                                }

                            }).catch (function (e) {
                                $log.error('Infrastructure Manager [%s] -> Error while remoteDoSnmp -> ', _this.activeConnection, e);
                            }));
                        ...
                        return $q.all(oid_promises).then(function () {
                            ...
                            // total octets
                            _this.getActiveConnection.data.totals.total_octets[0].unshift({
                                x: _this.getActiveConnection.data.totals.total_octets[0].length,
                                y: _this.getActiveConnection.data.totals.total_octets_in
                            });
                            _this.getActiveConnection.data.totals.total_octets[1].unshift({
                                x: _this.getActiveConnection.data.totals.total_octets[1].length,
                                y: -_this.getActiveConnection.data.totals.total_octets_out
                            });

                            // total iface packets
                            _this.getActiveConnection.data.totals.total_packets.unshift({
                                x: _this.getActiveConnection.data.totals.total_packets.length,
                                y: _this.getActiveConnection.data.totals.total_packets_sum
                            });

                            // total iface packets loss
                            _this.getActiveConnection.data.totals.total_packets_loss.unshift({
                                x: _this.getActiveConnection.data.totals.total_packets_loss.length,
                                y: _this.getActiveConnection.data.totals.total_packets_loss_sum
                            });

                            // Call function each 5 seconds
                            $timeout(function () {
                                getInterfacesStatistics();
                            }, 20000);
                    ...
                    var getInterfacesData = function () {
                        ...
                        angular.forEach(_this.getActiveConnection.data.ifaces, function (iface) {
                            var ifaceOIDs = {
                                oid: [
                                    '1.3.6.1.2.1.2.2.1.2.' + iface.id, // ifDescr
                                    '1.3.6.1.2.1.2.2.1.3.' + iface.id, // ifType
                                    '1.3.6.1.2.1.2.2.1.4.' + iface.id, // ifMTU
                                    '1.3.6.1.2.1.2.2.1.5.' + iface.id, // ifSpeed
                                    '1.3.6.1.2.1.2.2.1.6.' + iface.id, // ifPhysAddress
                                    '1.3.6.1.2.1.2.2.1.7.' + iface.id, // ifAdminStatus
                                    '1.3.6.1.2.1.2.2.1.8.' + iface.id, // ifOperStatus
                                    // ifMIB
                                    '1.3.6.1.2.1.31.1.1.1.1.' + iface.id, // ifName
                                    '1.3.6.1.2.1.31.1.1.1.18.' + iface.id // ifAlias
                                ],
                                type: 'get'
                            };
                            ...
                            oid_promises.push(ServerFactory.remoteDoSnmp(_this.activeConnection, ifaceOIDs.oid, ifaceOIDs.type).then(function (data) {
                                ...
                                iface.info = data.data.data.data;
                    ...
                    var getInterfacesOIDs = function (parentOID) {
                        ...
                        return ServerFactory.remoteDoSnmp(_this.activeConnection, parentOID.oid, 'get-next').then(function (data) {
                            ...
                            if (data.data.data.data[0].oid.startsWith('1.3.6.1.2.1.2.2.1.1')) {
                                ...
                                _this.getActiveConnection.data.ifaces.push({
                                    id: data.data.data.data[0].value
                                });
                    ...
                    var getMainSNMPData = function () {
                        ...
                        _this.getActiveConnection.data.totals = [];
                        _this.getActiveConnection.data.totals.total_octets = [[{
                            x: 0,
                            y: 0
                        }],[{
                            x: 0,
                        ...
                        _this.getActiveConnection.data.totals.total_packets_loss = [{
                            x: 0,
                            y: 0
                        }];
                        _this.getActiveConnection.data.totals.total_packets = [{
                            x: 0,
                            y: 0
                        }];
