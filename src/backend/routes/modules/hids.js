...
module.exports = function init (conn) {
	...
	var doWithLine = function (line) {
		return new Promise(function (resolve, reject) {
			...
			if (type !== "name" && (matched === 2 || matched === 3)) { current_result.lines.push({"line": line, "result": "not needed"}); return resolve(); }
		  ...
		  for (var x = 0; x < segments.length; x++) {
		  	regex = /^(!?r|!?=|!?<|!?>):(.+)$/g;
			    ...
			    if (m.index === regex.lastIndex) {regex.lastIndex++;}
			    if (m[1] === "r" || m[1] === "!r") {regexconditions.push({"type": m[1], "regex": m[2]});}
				}
		  }
		  ...
		  if (type == "file") {
		  	return globals.execAsync('cat ' + first).then(function (data) {
			  	...
			  	if (fileContents.length === 0) {matched = 0;}
			  	...
			  	for (var l = 0; l < fileContents.length; l++) {
			  		...
			  		for (var r = 0; r < regexconditions.length; r++) {
			  			if (regexconditions[r].type.charAt(0) === "r" && fileContents[l].match(regexconditions[r].regex)) {++total_match;}
			  			else if (regexconditions[r].type.charAt(0) === "!" && !fileContents[l].match(regexconditions[r].regex)) {++total_match;}
			  			else {break;}
			  		}

			  		if (total_match === regexconditions.length) {matched = 1; break;}
			  	}

			  	if ((condition === "any" || condition === "any required") && matched === 1) {matched = 3; current_result.lines.push({"line": line, "result": "passed"}); results.push(current_result);}
			  	else if ((condition === "any" || condition === "any required") && matched === 0) {matched = 5; current_result.lines.push({"line": line, "result": "not passed right now"}); }
			  	else if ((condition === "all" || condition === "all required") && matched === 0) {matched = 2; current_result.lines.push({"line": line, "result": "not passed"}); results.push(current_result);}
			  	else if ((condition === "all" || condition === "all required") && matched === 1) {current_result.lines.push({"line": line, "result": "passed right now"});}
			  	else if (condition === "none" && matched === 1) {matched = 2; current_result.lines.push({"line": line, "result": "not passed"}); results.push(current_result);}
			  	else if (condition === "none" && matched === 0) {matched = 4; current_result.lines.push({"line": line, "result": "passed right now"});}
			  	else {matched = 5; current_result.lines.push({"line": line, "result": "not passed right now"});}
	...
	this.run_hids = function (program) {
		...
		return new Promise(function (resolve, reject) {
			...
			promiseWhile(function() {
			  ...
			  return doWithLine(lines[z]).then(function () {z++;});
			}).then(function() {
