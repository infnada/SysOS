...
import {AnyOpsOSKubernetesSessionStateModule} from './anyopsos-module-kubernetes-session-state';
import {AnyOpsOSKubernetesDataRefresherModule} from './anyopsos-module-kubernetes-data-refresher';

import {IsomorphicWs} from './types/isomorphic-ws';

export class AnyOpsOSKubernetesModule {

  private readonly KubernetesSessionStateModule: AnyOpsOSKubernetesSessionStateModule;
  private readonly KubernetesDataRefresherModule: AnyOpsOSKubernetesDataRefresherModule;
  ...
  constructor(private readonly userUuid: string,
    ...
    this.KubernetesSessionStateModule = new AnyOpsOSKubernetesSessionStateModule(this.userUuid, this.sessionUuid, this.workspaceUuid, this.connectionUuid);
    this.KubernetesDataRefresherModule = new AnyOpsOSKubernetesDataRefresherModule(this.userUuid, this.sessionUuid, this.workspaceUuid, this.connectionUuid);
  ...
  newConnection(): Promise<BackendResponse> {
    ...
    return this.KubernetesSessionStateModule.createSession().then((kc: KubeConfig) => {

      this.KubernetesDataRefresherModule.createConnectionFolders();
      ...
      apiUrls.forEach((url: string) => {
        ...
        watch.watch(url,
          ...
          (phase: string, obj: any) => {
            ...
            this.KubernetesDataRefresherModule.parseObjects(obj);
          },
          (e: Error) => {
            throw e;
          });
  ...
  execToTerminal(terminalUuid: string, namespace: string, pod: string, container: string, command: string | string[] = '/bin/sh'): Promise<void> {
    ...
    const emitData = (terminalUuid: string, data: string) => this.TerminalsModule.terminalStout(terminalUuid, data);
