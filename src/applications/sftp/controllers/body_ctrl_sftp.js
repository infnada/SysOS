		...
		function ($scope, $element, sftpFactory, ServerFactory, Upload, fileSystemFactory, $timeout, cmanagerFactory, ApplicationsFactory, connectionsFactory, modalFactory) {
			...
			*/
			...
			this.deleteConnection = function (connection) {
				...
				var modalInstanceRemoveConnection = modalFactory.openLittleModal('Delete connection ' + (_this.getActiveConnection().description ? _this.getActiveConnection().description : _this.getActiveConnection().hostname), 'Remove the selected connection from the inventory?', '.window--sftp .window__main', 'question');
